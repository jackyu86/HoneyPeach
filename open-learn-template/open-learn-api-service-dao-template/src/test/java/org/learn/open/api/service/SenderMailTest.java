...
public class SenderMailTest {
	...
	public void sendAttachmentsEmail() throws MessagingException{
		...
		//附件1,获取文件对象.
		FileSystemResource file1 = new FileSystemResource(new File("/platform/spring-boot-mail/shareFolder/PMP记忆术.txt"));
		//添加附件，这里第一个参数是在邮件中显示的名称，也可以直接是head.jpg，但是一定要有文件后缀，不然就无法显示图片了。
		helper.addAttachment("pmp记忆术.txt", file1);
		//附件2
		FileSystemResource file2 = new FileSystemResource(new File("/platform/spring-boot-mail/shareFolder/卡内基沟通与人际关系.pdf"));
    ...
    public void sendTemplateMail() throws Exception {
        ...
        MimeMessageHelper helper = new MimeMessageHelper(mimeMessage, true);
        //基本设置.
  		helper.setFrom("aaa@qq.com");//发送者.
  		helper.setTo("bbb@qq.com");//接收者.
  		helper.setSubject("模板邮件（邮件主题）");//邮件主题.
        Map<String, Object> model = new HashMap<String, Object>();
        model.put("username", "aa");
