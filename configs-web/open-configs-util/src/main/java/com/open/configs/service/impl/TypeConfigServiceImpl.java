...
public class TypeConfigServiceImpl implements TypeConfigService {
	...
	private static final Logger LOG = LoggerFactory.getLogger(TypeConfigServiceImpl.class);
	
	/**
	 * zookeeper连接客户端
	 */
	protected OpenZookeeperClient zookeeperClient;
	...
	public void setZookeeperClient(
			OpenZookeeperClient zookeeperClient) {
	...
	private <T extends BaseConfig> void checkReget(final T oldResult, final String path, final JavaType type) {
		synchronized (this) {
			try {
				...
				if (com.open.configs.core.OpenZookeeperClient.pathShouldReget(path) || c.getZother().checkReget()) {
								...
								while (!flag) {
									...
									try {
										...
										if (resultZk != null) {
											...
											com.open.configs.core.OpenZookeeperClient.changeReget(path, false);
										}
