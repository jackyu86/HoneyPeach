        ...
        for (final PathListener listener : listeners) {
            ...
            for(int i=0; i<3; i++){
	            try {
                    ...
                    Map<String,String> mapData = new ConcurrentHashMap<String,String>();
	                listener.ZkDataListener(mapData);
	                break;
	            } catch (ZkException e) {
    ...
    public  byte[] readData(final String path, final Stat stat, final boolean watch) {
		...
		try {
           ...
           String json = mapper.writeValueAsString(data);
            Map<String,String> mapdata = mapper.convertValue(json,HashMap.class);
		} catch (KeeperException e) {
